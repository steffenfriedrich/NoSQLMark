 ...
 */
class MyIntegrationSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender with WordSpecLike with Matchers with BeforeAndAfterAll {

  def this() = this(ActorSystem("NoSQLMarkCluster", MyIntegrationSpec.config.getConfig("client").withFallback(MyIntegrationSpec.config)))
  ...
  "Some benchmark jobs" must {
    "return results" in {
      ...
      // transactional phase
      val transactionalJob = loadJob.asInstanceOf[CoreJob].copy(
